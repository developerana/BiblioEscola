...
export default function Dashboard() {
  ...
  return <MainLayout>
      ...
      {canManageBooks ? <Card className="shadow-card mb-8">
          <CardHeader>
        ...
        </Card> : <Card className="shadow-card mb-8">
          <CardHeader>
            <CardTitle className="font-display text-xl flex items-center gap-2">
          ...
          <CardContent>
            {loading ? <div className="flex items-center justify-center py-8">
              ...
              </div> : recentLoans.length === 0 ? <p className="text-muted-foreground text-center py-8">
                Nenhum empréstimo registrado ainda.
              </p> : <div className="space-y-4">
                {recentLoans.map(loan => {
            const isOverdue = loan.expected_return_date && !loan.actual_return_date && isBefore(parseISO(loan.expected_return_date), new Date());
            return <div key={loan.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-lg bg-muted/50 border border-border">
                      ...
                      <div className="flex items-center gap-3 justify-between sm:justify-end">
                        ...
                        <StatusBadge status={loan.actual_return_date ? 'devolvido' : isOverdue ? 'atrasado' : 'emprestado'} />
                      </div>
                    </div>;
          })}
              </div>}
